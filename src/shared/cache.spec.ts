...
describe('cache utils', () => {
  ...
  describe('concurrent getCache calls', () => {
    beforeEach(() => {
      _resetCache();
      _resetCacheCheck();
    });
    ...
    it('returns the same promise instance for concurrent calls', () => {
      const promise1 = getCache();
      const promise2 = getCache();
      const promise3 = getCache();

      expect(promise1).toBe(promise2);
      expect(promise2).toBe(promise3);
    });

    it('resolves all concurrent calls to the same cache instance', async () => {
